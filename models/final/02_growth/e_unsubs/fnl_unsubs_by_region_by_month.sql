...
regional_monthly_unsubs as (
    select
        ...
        SUM(CASE WHEN Country = 'CA' AND (City ilike '%None%' OR City IS NULL) THEN 1 ELSE 0 END) AS CA_None,
        ...
        SUM(CASE WHEN Country = 'US' AND (City ilike '%None%' OR City IS NULL) THEN 1 ELSE 0 END) AS US_None
    from unsubs
    GROUP BY 1
)
